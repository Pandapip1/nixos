{
  config,
  lib,
  pkgs,
  ...
}:

{
  users.users.gavin = {
    enable = lib.mkDefault true;
    isNormalUser = true;
    useDefaultShell = true;
    createHome = true;
    description = "gavin";
    extraGroups = [
      "networkmanager"
      "wheel"
      "disk"
      "corectrl"
      "dialout"
      "podman"
      "plugdev"
      "audio"
      "pipewire" # https://github.com/NixOS/nixpkgs/issues/466987
      "pcscd"
      "input"
      # `brightnessctl` (below) needs this to write to backlight sysfs
      # without root — its own bundled udev rules grant the "video" group
      # write access, matching that package's standard NixOS usage.
      "video"
    ]
    ++ lib.optional config.services.kubo.enable config.services.kubo.group;
    # `brightnessctl`: mudhuts' own brightness-up/down media keys shell
    # out to it directly (see mudhuts/src/input.rs's Action::Brightness*
    # handling) rather than talking to backlight sysfs itself.
    packages = with pkgs; [ brightnessctl ];
    openssh = {
      authorizedKeys.keys = [
        "ssh-rsa  cardno:32_112_499"
        "ecdsa-sha2-nistp384 AAAAE2VjZHNhLXNoYTItbmlzdHAzODQAAAAIbmlzdHAzODQAAABhBP3SiXcjHQsN+QmYdTGvOENgjJ8FbsCKHeBnmKg/60VItK/+254fGMqr27HHSaRlT6nIWODfMf6Ze4cIi4X2g9e9C5gDwLiurzY3o+AzBNQXQdbrdg1JeEqTwSWHCAl8yw== openpgp:0x18BAD638"
      ];
    };

    # changeme
    initialPassword = "changeme";
  };
  home-manager.users.gavin.home.stateVersion = "25.11";
}
