...
lib.mkIf (config.services.graphical-desktop.enable && !(config.optimizations.lean.enable)) {
  # Bypassing programs.vscode since it hardcodes its finalPackage into
  # environment.systemPackages; building vscode-with-extensions directly lets
  # us route it through the singleton profile instead, same as firefox.nix.
